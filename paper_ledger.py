...
class PaperLedger:
    ...
    @staticmethod
    def _buy_total(trades):
        return sum(float(t.get("notional", 0.0)) for t in trades if t.get("action") == "BUY")

    def total_buy_count(self):
        return sum(1 for t in self.trades if t.get("action") == "BUY")
    ...
    def _load(self):
        ...
        try:
            ...
            self.total_spent = float(d.get("total_spent", 0.0))
            if self.total_spent <= 0.0:
                self.total_spent = self._buy_total(self.trades)
