...
def _normalize(m):
    ...
    return {'id':str(m.get('id') or condition),'condition':condition,'market':str(m.get('question') or m.get('title') or slug),'slug':slug,'asset':asset,'up':up,'down':down,'start_ts':start,'end_ts':end,'raw':m,'accepting_orders':bool(m.get('acceptingOrders',True)),'enable_order_book':bool(m.get('enableOrderBook',True))}
