...
def test_v161_bot_banner_and_strategy_version():
    assert 'V17 FULL-SCALE TRADER REPLICA 40PCT' in Path(__file__).resolve().parents[1].joinpath('bot.py').read_text()



def test_v17_10_dollar_paper_defaults_are_locked_in_source():
    src = Path(__file__).resolve().parents[1].joinpath("bot.py").read_text()
    assert 'STARTING_CAPITAL", "10"' in src
    assert 'MAX_TOTAL_EXPOSURE", "10"' in src
    assert 'PAPER_TRADING", "true"' in src
    assert 'BET #{bet_no}' in src
    assert 'TOTAL_SPENT=${cumulative_spent:.2f}' in src
    assert 'CASH_REMAINING=${cash_remaining:.2f}' in src



def test_v17_trade_log_schema_contains_paper_spend_fields():
    src = Path(__file__).resolve().parents[1].joinpath("research_logger.py").read_text()
    assert '"bet_number"' in src
    assert '"cumulative_spent"' in src
    assert '"bankroll_used_pct"' in src


def test_v17_paper_spend_persists_across_ledger_reload():
    with tempfile.TemporaryDirectory() as td:
        path = Path(td) / "paper_state.json"
        ledger = PaperLedger(path, 10)
        ledger.buy("c1", "t1", "m1", "Up", .50, 1.25, 1000)
        ledger.buy("c2", "t2", "m2", "Down", .25, 0.75, 1001)
        assert ledger.total_spent == pytest.approx(2.0)
        assert ledger.total_buy_count() == 2
        reloaded = PaperLedger(path, 10)
        assert reloaded.total_spent == pytest.approx(2.0)
        assert reloaded.total_buy_count() == 2
        assert reloaded.cash == pytest.approx(8.0)
...
def test_v17_execution_speed_settings_are_subsecond_and_bounded():
    src = Path(__file__).parents[1].joinpath("bot.py").read_text()
    assert 'LOOP_SECONDS", "0.25"' in src
    assert 'DISCOVERY_INTERVAL_SECONDS", "5"' in src
    assert 'CADENCE_FALLBACK_SECONDS", "2"' in src
    assert "ThreadPoolExecutor" in src


def test_v17_scaled_paper_budget_limits_bet_and_open_exposure():
    from paper_risk import paper_budget_target
    x = paper_budget_target(
        2.12, bankroll=10, cash=10, open_exposure=0,
        target_scale=0.10, max_bet_fraction=0.10,
        max_open_fraction=0.50, absolute_open_cap=10,
    )
    assert x['scaled_target'] == pytest.approx(0.212)
    assert x['spend'] == pytest.approx(0.212)
    y = paper_budget_target(
        2.12, bankroll=10, cash=10, open_exposure=4.95,
        target_scale=0.10, max_bet_fraction=0.10,
        max_open_fraction=0.50, absolute_open_cap=10,
    )
    assert y['open_cap'] == pytest.approx(5.0)
    assert y['spend'] == pytest.approx(0.05)


def test_v17_scaled_paper_budget_respects_cash_and_bet_cap():
    from paper_risk import paper_budget_target
    x = paper_budget_target(
        22.20, bankroll=10, cash=0.80, open_exposure=4,
        target_scale=0.10, max_bet_fraction=0.10,
        max_open_fraction=0.50, absolute_open_cap=10,
    )
    assert x['scaled_target'] == pytest.approx(2.22)
    assert x['max_bet'] == pytest.approx(1.0)
    assert x['remaining_open'] == pytest.approx(1.0)
    assert x['spend'] == pytest.approx(0.80)


def test_v17_cadence_fallback_does_not_spin_after_due_event():
    src = Path(__file__).resolve().parents[1].joinpath('bot.py').read_text()
    assert 'Never let one cadence event remain permanently due' in src
    assert 'cadence_due_since = None' in src


def test_v17_paper_drawdown_halt():
    from paper_risk import drawdown_halt
    assert not drawdown_halt(8.0, 10.0, 0.30)
    assert drawdown_halt(7.0, 10.0, 0.30)


def test_v17_scaled_experiment_defaults_are_present():
    src = Path(__file__).resolve().parents[1].joinpath('bot.py').read_text()
    assert 'PAPER_TARGET_SCALE = bounded_float("PAPER_TARGET_SCALE", 0.10' in src
    assert 'MAX_BET_BANKROLL_PCT = bounded_float("MAX_BET_BANKROLL_PCT", 0.10' in src
    assert 'MAX_OPEN_EXPOSURE_PCT = bounded_float("MAX_OPEN_EXPOSURE_PCT", 0.50' in src
    assert 'PAPER_MAX_DRAWDOWN_PCT = bounded_float("PAPER_MAX_DRAWDOWN_PCT", 0.30' in src


def test_v17_scaled_paper_budget_preserves_relative_target_shape():
    from paper_risk import paper_budget_target
    targets = [0.58, 0.42, 0.22]
    spends = [paper_budget_target(
        t, bankroll=10, cash=10, open_exposure=0,
        target_scale=0.10, max_bet_fraction=0.10,
        max_open_fraction=0.50, absolute_open_cap=10,
    )['spend'] for t in targets]
    assert spends[0] / spends[1] == pytest.approx(targets[0] / targets[1])
    assert spends[1] / spends[2] == pytest.approx(targets[1] / targets[2])


def test_v17_experiment_exposure_defaults_to_half_bankroll():
    src = Path(__file__).resolve().parents[1].joinpath('bot.py').read_text()
    assert 'MAX_OPEN_EXPOSURE_PCT = bounded_float("MAX_OPEN_EXPOSURE_PCT", 0.50' in src
    assert 'EFFECTIVE_MAX_TOTAL_EXPOSURE' in src


def test_v17_cumulative_spend_is_not_used_as_bankroll_usage_metric():
    src = Path(__file__).resolve().parents[1].joinpath('bot.py').read_text()
    assert 'CUMULATIVE_SPEND=${cumulative_spent:.2f}' in src
    assert 'OPEN_EXPOSURE_PCT=' in src
    assert 'BANKROLL_USED=' not in src
